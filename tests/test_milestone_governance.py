...
def registry(command=None):
    return {"milestones": [{
        "id": "KB-M0", "title": "Controller", "status": "not_started", "intent": "Prove evidence flow.",
        "dependencies": [], "entry_conditions": ["Registry exists."],
        "proof_contract": {"real_surface": "A real local subprocess.",
                           "capture_command": command or [sys.executable, "-c", "print('KB_MILESTONE_CONTROLLER_VALID')"],
                           "required_markers": ["KB_MILESTONE_CONTROLLER_VALID"], "freshness_hours": 1,
...
def test_dependencies_and_static_completion_are_rejected(tmp_path):
    ...
    dependent["milestones"].append({
        "id": "KB-M1", "title": "Dependent", "status": "not_started", "intent": "Wait for M0.",
        "dependencies": ["KB-M0"], "entry_conditions": ["M0 complete."],
        "proof_contract": {"real_surface": "A real local subprocess.", "capture_command": [sys.executable, "-c", "print('DEPENDENT')"],
                           "required_markers": ["DEPENDENT"], "freshness_hours": 1, "verifier": "registry_self_check"}
    })
    state = {"version": 1, "milestones": {}}
...
def test_independent_verifier_is_required_for_non_self_check(tmp_path):
    ...
    result = governance.verify_milestone(definitions, state, "KB-M0", tmp_path,
                                          {"remote_readback": lambda root: {"real_surface_checked": True}})
